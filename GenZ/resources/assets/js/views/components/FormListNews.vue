        ...
        <div class="col-12">
          <b-card header="<i class='fa fa-align-justify'></i> Loại danh mục">
            <table class="table table-bordered table-striped table-sm table-responsive">
              ...
              <tbody>
                <tr v-for="listnew in listnews">
                  <td>{{listnew.id}}</td>
                  <td class="text-center">
                    <img :src="listnew.list_icon" class="img-responsive" alt="No image" width="50" height="50">
                  </td>
                  <td>{{listnew.list_name}}</td>
                  <td>{{listnew.mod_name}}</td>
                  <td>{{listnew.list_description}}</td>
                  <td>{{listnew.list_number}}</td>
                  <td v-if="listnew.list_type == 0" class="text-center">
                    <span class="badge badge-warning">Đang Ẩn</span>
                  </td>
                  <td v-else-if="listnew.list_type == 1" class="text-center">
                    <span class="badge badge-success">Đang Hiện</span>
                  </td>
                  <td v-else="listnew.list_type == 2" class="text-center">
                    <span class="badge badge-info">HOT</span>
                  </td>
                  <td class="text-center">
                    <b-button type="button" variant="primary" @click="updatemod_btn(listnew.id)"> Sửa</b-button>
                    <b-button variant="danger" @click="remove(listnew.id)">Xóa</b-button>
                  </td>
                </tr>
              </tbody>
            </table>
...
export default {
  ...
  methods:{    
    ...
    addnewmod_click(){
      ...
      this.loading = false;
      axios.post('/api/listnews', { name:this.list_name,id_modnews:this.id_mod,number:this.list_number,type:this.list_type,description:this.list_description,image:this.image }).then(response => {
        $("#add_name").val('');
        $("#add_mod").val('');
        $("#add_number").val('');
        $("#add_type").val('');
    ...
    updatenewmod(id){
      this.loading = true;
      var url = '/api/listnews/'+id;
      axios.post(url, { name:this.edit_item.list_name,id_modnews:this.edit_item.id_mod,number:this.edit_item.list_number,type:this.edit_item.list_type,description:this.edit_item.list_description,image:this.eimage }).then(response => {
          $("#id_name").val('');
          $("#id_number").val('');
          $("#id_type").val('');
          $("#id_icon").val('');
    ...
    remove(id){
      ...
      .then(res => {
        if (res.ok) {
          this.loading = true;
          var url = '/api/listnews/remove/'+id;
