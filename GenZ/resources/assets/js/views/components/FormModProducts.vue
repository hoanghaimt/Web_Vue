              ...
              <tbody>
                <tr v-for="modproduct in modproducts">
                  ...
                  <td class="text-center">
                    <img :src="modproduct.mod_icon" class="img-responsive" alt="no imagem" width="50" height="50">
                  </td>
...
export default {
  ...
  methods:{    
    ...
    addnewmod_click(){
      this.loading = false;
      axios.post('/api/modproducts', { name:this.mod_name,number:this.mod_number,type:this.mod_type,description:this.mod_description,image:this.image }).then(response => {
        this.getallmod();
        $("#add_name").val('');
        $("#add_number").val('');
        $("#add_type").val('');
