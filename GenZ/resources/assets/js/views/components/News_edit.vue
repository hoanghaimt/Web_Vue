      ...
      <div class="col-lg-12">
        <b-card>
          ...
          <div class="row">
            ...
            <div class="col-3">
              <b-form-fieldset label="Hình ảnh cũ">
                <div>
                    <img :src="news_image" class="img-responsive text-center text-justify" height="140" width="140">
                </div>
              </b-form-fieldset>
              <b-form-fieldset label="Hình ảnh bản tin">
                <div v-if="image">
            ...
            <div class="col-12">
              <b-form-fieldset label="Tóm tắt bản tin" :horizontal="false">
                <vue-ckeditor v-model="news_intro" :required="true" id="add_intro" name="news_intro" :config="config" />
                <br><span class="error_msg" v-if="errors.intro">{{errors.intro[0]}}</span>
              </b-form-fieldset>
            </div>
            <div class="col-12">
              <b-form-fieldset label="Nội dung bản tin" :horizontal="false">
                <vue-ckeditor v-model="news_content" :required="true" id="add_content" name="news_content" :config="config" />
          ...
          <div class="form-actions">
            <b-button type="button" variant="success" @click="postupdatenews()" >Lưu bản tin </b-button>
...
export default {
  ...
  mounted(){
    ...
    axios.get(this.url+'/edit/'+this.$route.params.news_id).then(response=> {
      var id_url = '/api/news/renderlistselect/'+response.data.id_listnews;
      axios.get(id_url).then(response => {
          $('#add_id_list').html(response.data);
        });
      this.news_name = response.data.news_name;
      this.news_video = response.data.news_video;
      this.news_image = response.data.news_image;
      this.news_tags = response.data.news_tags;
      this.news_keywords = response.data.news_keywords;
      this.news_intro = response.data.news_intro;
      this.news_content = response.data.news_content;
    });
  ...
  methods:{    
    ...
    postupdatenews(){
      ...
      axios.post('/api/news/'+id, { name:this.news_name,video:this.news_video,list_id:this.id_list,keywords:this.news_keywords,tags:this.news_tags,image:this.image,intro:this.news_intro,content:this.news_content}).then(response => {
